...
@Component({
    ...
    <div class="obsidian-layout">
      ...
      <section class="orbit-hero">
        <div class="orbit-visual" role="button" tabindex="0" (click)="goToSystem()" (keydown.enter)="goToSystem()">
          <div class="glow-ring" [class.urgent-glow]="studyState.dueCount() > 0"></div>
          <svg class="ring-svg" width="260" height="260" aria-hidden="true">
            ...
            <circle class="progress" cx="130" cy="130" r="110"
                    [class.urgent-stroke]="studyState.dueCount() > 0"
          ...
          <div class="core-data">
            @if (studyState.dueCount() > 0) {
              <div class="active-core urgent">
                ...
                <button class="resume-btn action-btn pulse" (click)="$event.stopPropagation(); goToSystem()">
                  START REVIEW
                </button>
              </div>
              ...
              <div class="active-core">
                ...
                <button class="resume-btn" (click)="$event.stopPropagation(); goToSystem()">
                  VIEW STATS
                </button>
              </div>
            }
  ...
  styles: [`
    ...
    .obsidian-layout { padding-bottom: var(--safe-bottom-and-footer); min-height: 100%; color: #E2E8F0; font-family: sans-serif; }
    ...
    .uplink-text { font-family: monospace; font-size: 0.65rem; font-weight: 700; color: #94a3b8; letter-spacing: 1px; }
    ...
    .glow-ring {
      ...
      filter: blur(20px); transition: background 0.3s;
    }
    .glow-ring.urgent-glow { background: radial-gradient(circle, rgba(59,130,246,0.3) 0%, transparent 75%); }
    ...
    .progress.urgent-stroke { stroke: #60A5FA; filter: drop-shadow(0 0 12px #60A5FA); stroke-width: 14; }
    ...
    .action-btn { background: #3B82F6; color: white; border: none; box-shadow: 0 4px 15px rgba(59,130,246,0.4); }
    ...
    .progress-line { height: 4px; background: rgba(255,255,255,0.15); border-radius: 2px; overflow: hidden; margin-bottom: 1rem; backdrop-filter: blur(4px); }
    ...
    .status-indicator { font-size: 0.6rem; font-weight: 800; padding: 4px 8px; border-radius: 4px; background: rgba(255,255,255,0.1); color: #64748B; letter-spacing: 1px; text-transform: uppercase; }
    .status-indicator.active { background: rgba(59, 130, 246, 0.2); color: #60A5FA; border: 1px solid rgba(59, 130, 246, 0.3); }
    .status-indicator.complete { background: rgba(16, 185, 129, 0.2); color: #10B981; border: 1px solid rgba(16, 185, 129, 0.3); }

    @keyframes pulse-red { 0% { opacity: 1; } 50% { opacity: 0.4; } 100% { opacity: 1; } }
    @keyframes pulse-shadow { 0% { box-shadow: 0 0 0 0 rgba(59,130,246,0.7); } 70% { box-shadow: 0 0 0 10px rgba(59,130,246,0); } 100% { box-shadow: 0 0 0 0 rgba(59,130,246,0); } }
  `]
})
export class DashboardComponent implements OnInit {
  ...
  dashOffset = computed(() => {
    ...
    const progress = hasDue ? 1 : Math.min(this.dailyProgress() / this.dailyGoal(), 1);
